...
void show_student_by_name()
{
	...
	while(temp!=NULL)
	{
		...
		if(strcmp(name,temp->sdata.name)==0)
		{
			break;
		}
		temp=temp->next;
	...
	printf("学号\t姓名\t密码\t数学\t语文\n");
		...
		printf("%d\t%s\t%s\t%.2f\t%.2f\n",temp->sdata.id,temp->sdata.name,temp->sdata.pwd,temp->sdata.math,temp->sdata.chinese);
